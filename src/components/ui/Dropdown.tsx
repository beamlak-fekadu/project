...
export default function Dropdown({ trigger, items, align = 'right' }: DropdownProps) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className={`absolute z-[75] mt-1 min-w-[180px] rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-900 ${align === 'right' ? 'right-0' : 'left-0'}`}>
          {items.map((item, i) => (
            <button
              key={i}
