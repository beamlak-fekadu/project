...
00068 — maintenance_events RLS hotfix: replaces `insert_maintenance_events` and `update_maintenance_events` policies (from 00012/00026) so bme_head can write reliability evidence on WO completion. Before this fix, BME Head completing a work order triggered "new row violates row-level security policy for table maintenance_events" because the original allowlist was admin/technician/developer (+store_user on INSERT only) and the application capability matrix had already given bme_head work_order.complete. UPDATE keeps the four-role list (developer/admin/bme_head/technician) — UPDATE is required because the action's idempotent completion-event upsert UPDATEs the existing completion-marked event on re-completion. INSERT additionally keeps store_user for offline replay paths that log maintenance_events notes from stock context. select_maintenance_events from 00012 (USING (true)) is unchanged.

NEVER modify 00001–00067. Next migration must be 00069.
